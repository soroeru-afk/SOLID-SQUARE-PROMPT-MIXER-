...
code = code.replace(
  /className=\{`group flex items-center gap-1\.5 px-3 py-0\.5 text-\[10px\] font-mono border rounded-sm cursor-pointer whitespace-nowrap transition-all \$\{\n\s*activeTabId === tab\.id \n\s*\? \(theme === 'light' \? 'bg-white border-border-hover text-text-main font-bold shadow-sm' : 'bg-bg-input border-border-hover text-text-main font-bold'\) \n\s*: 'bg-transparent border-transparent text-text-dim hover:bg-bg-input hover:text-text-main'\n\s*\}\`\}/,
