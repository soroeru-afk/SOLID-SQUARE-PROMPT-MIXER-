...
const regex = /className=\{\`p-2 rounded flex items-center space-x-2 cursor-pointer transition-colors group relative \$\{\s*isSelected \? 'bg-bg-surface border border-blue-500\/30' : 'bg-bg-input border border-border-main hover:border-border-hover'\s*\}\`\}/m;
...
if (regex.test(code)) {
    ...
    console.log("Fixed VariationColumn background classes");
} else {
