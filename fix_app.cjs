...
// I will extract the exact code using a robust AST or just index search.
...
if (startIdx !== -1) {
  ...
  for (let i = startIdx; i < code.length; i++) {
    if (code[i] === '{') bracketCount++;
      ...
      bracketCount--;
      if (bracketCount === 0 && code.substr(i, 45).includes('}, [setEditorText, setNegativeEditorText]);')) {
        endIdx = i + 45; // roughly
        break;
      }
    }
  ...
  // Actually, I can just use a simpler regex. Let's just find the first occurrence of the end string after startIdx.
  ...
  if (actualEndIdx !== -1) {
    ...
    // Also remove any duplicated handleMixAttributes below just in case.
    const secondStartIdx = code.indexOf(startStr, startIdx + replacement.length);
    if (secondStartIdx !== -1) {
      const secondEndIdx = code.indexOf(endStr, secondStartIdx);
      if (secondEndIdx !== -1) {
        code = code.substring(0, secondStartIdx - 2) + code.substring(secondEndIdx + endStr.length);
      }
    }
